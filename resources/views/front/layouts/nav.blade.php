            ...
            <ul class="navbar-nav mr-5">
                <li class="nav-item dropdown">
                  ...
                  <div class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
                    ...
                    <span><a class="dropdown-item" href= {{route('single',$item->productID)}}><div class = "container"><img src="{{asset('images/').'/'.$item->products->coverPhoto}}" alt="" width="40">
                       {{$item->products->name}}</div>	&nbsp  Quantity: {{$item->productQty}}  	&nbsp Price:    ${{($item->productQty)*($item->products->price)}} </a></span>
                    @endforeach
